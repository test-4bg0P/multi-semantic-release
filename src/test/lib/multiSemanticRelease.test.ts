...
describe('multiSemanticRelease()', () => {
  ...
  test('Initial commit (changes in all packages) concurrency', async () => {
    // Create Git repo with copy of Yarn workspaces fixture.
    const cwd = gitInit()
    copyDirectory(`src/test/fixtures/yarnWorkspacesBatch/`, cwd)
    const sha = gitCommitAll(cwd, 'feat: Initial release')
    gitInitOrigin(cwd)
    gitPush(cwd)
    ...
    expect(err).toBe(false)
    const out = stdout.getContentsAsString('utf8')
    expect(out).toMatch('Started multirelease! Loading 7 packages...')
    expect(out).toMatch('Loaded package msr-test-a')
    expect(out).toMatch('Loaded package msr-test-b')
    expect(out).toMatch('Loaded package msr-test-c')
    expect(out).toMatch('Loaded package msr-test-d')
    expect(out).toMatch('Loaded package msr-test-e')
    expect(out).toMatch('Loaded package msr-test-f')
    expect(out).toMatch('Loaded package msr-test-g')
    expect(out).toMatch('Queued 7 packages! Starting release...')
    expect(out).toMatch('Created tag msr-test-a@1.0.0')
    expect(out).toMatch('Created tag msr-test-b@1.0.0')
    expect(out).toMatch('Created tag msr-test-c@1.0.0')
    expect(out).toMatch('Created tag msr-test-d@1.0.0')
    expect(out).toMatch('Created tag msr-test-e@1.0.0')
    expect(out).toMatch('Created tag msr-test-f@1.0.0')
    expect(out).toMatch('Created tag msr-test-g@1.0.0')
    expect(out).toMatch('Released 7 of 7 packages, semantically!')
    ...
    expect(result[2].result.nextRelease.notes).not.toMatch('### Dependencies')
    ...
    expect(result[4].result.nextRelease.notes).not.toMatch('### Dependencies')

    // F.
    expect(result[5].name).toBe('msr-test-f')
    expect(result[5].result.lastRelease).toEqual({})
    expect(result[5].result.nextRelease).toMatchObject({
      gitHead: sha,
      gitTag: 'msr-test-f@1.0.0',
      type: 'minor',
      version: '1.0.0',
    })
    expect(result[5].result.nextRelease.notes).toMatch('# msr-test-f 1.0.0')
    expect(result[5].result.nextRelease.notes).toMatch(
      '### Features\n\n* Initial release',
    )
    expect(result[5].result.nextRelease.notes).not.toMatch('### Dependencies')

    // G.
    expect(result[6].name).toBe('msr-test-g')
    expect(result[6].result.lastRelease).toEqual({})
    expect(result[6].result.nextRelease).toMatchObject({
      gitHead: sha,
      gitTag: 'msr-test-g@1.0.0',
      type: 'minor',
      version: '1.0.0',
    })
    expect(result[6].result.nextRelease.notes).toMatch('# msr-test-g 1.0.0')
    expect(result[6].result.nextRelease.notes).toMatch(
      '### Features\n\n* Initial release',
    )
    expect(result[6].result.nextRelease.notes).not.toMatch('### Dependencies')
